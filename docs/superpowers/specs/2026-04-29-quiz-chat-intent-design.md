...
Triggered when Stage 2 does not match. Single LLM call. Inputs: `message`, `question`, `options[]`. The LLM determines language match itself by comparing the language of `message` against the language of `question` — no separate detection step.
